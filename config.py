...
# # # NYUHHA_Dataset config
# """Dataset Path"""
# C.dataset_name = 'NYUDepthv2HHA'
# C.dataset_path = osp.join(C.root_dir, 'datasets', 'NYUDepthv2HHA')
...
# C.train_source = osp.join(C.dataset_path, "train_ids.txt")
# C.eval_source = osp.join(C.dataset_path, "test_ids.txt")   ###test_ids
# C.is_test = False
...
# True when label 0 is invalid, you can also modify the function _transform_gt in dataloader.RGBXDataset
# True for most dataset valid, Faslse for MFNet(?)
C.x_root_folder = osp.join(C.dataset_path, 'Depth_resize')
C.x_format = '.png'
C.x_is_single_channel = True # True for raw depth, thermal and aolp/dolp(not aolp/dolp tri) input
C.train_source = osp.join(C.dataset_path, "train2.txt")
C.eval_source = osp.join(C.dataset_path, "test2.txt")
...
"""Train Config"""
# C.lr = 6e-5 #batchsize=8
# C.lr = 4.5e-5 #batchsize=6
# C.lr = 4e-5 #batchsize=4 250epoch
C.lr = 3e-5 #batchsize=4
# C.lr = 1.5e-5 #batchsize=2
# C.lr = 7.5e-6 #batchsize=1
C.lr_power = 0.9
C.momentum = 0.9
C.weight_decay = 0.01
C.batch_size = 4
...
# C.checkpoint_step = 25
# C.lr = 4e-5  # 原 3e-5
# C.warm_up_epoch = 5  # 原 10
# # C.lr_power = 0.8  # 可選，略加快 decay 曲線