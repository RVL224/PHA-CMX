...
from ..net_utils import PHA  # 改為 PHA
# from ..net_utils import SHA  # ← 已移除
...
class DecoderHead(nn.Module):
    def __init__(self,
                 ...
                 embed_dim=768,
                 align_corners=False,
                 use_pha=[True, True, False, False],  # 依層啟用 PHA
                 input_resolutions=[(128, 128), (64, 64), (32, 32), (16, 16)]):  # 每層對應解析度
        ...
        self.pha_c4 = PHA(embed_dim, embed_dim, input_resolution=input_resolutions[3]) if use_pha[0] else nn.Identity()
        self.pha_c3 = PHA(embed_dim, embed_dim, input_resolution=input_resolutions[2]) if use_pha[1] else nn.Identity()
        self.pha_c2 = PHA(embed_dim, embed_dim, input_resolution=input_resolutions[1]) if use_pha[2] else nn.Identity()
        self.pha_c1 = PHA(embed_dim, embed_dim, input_resolution=input_resolutions[0]) if use_pha[3] else nn.Identity()

        self.linear_fuse = nn.Sequential(
            nn.Conv2d(in_channels=embed_dim * 4, out_channels=embed_dim, kernel_size=1),
