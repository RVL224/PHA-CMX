...
from ..net_utils import FeatureFusionPHA as FFM
...
#                  norm_layer=nn.LayerNorm,       # 預設 LayerNorm
#                  norm_fuse=None,                # ← 預設 None，稍後自動繼承
#                  depths=[3, 4, 6, 3],
#                  sr_ratios=[8, 4, 2, 1]):       # ★ 這裡最後一個右括號再接冒號
#         super().__init__()

#         # 若未指定 norm_fuse，就沿用外層傳進來的 norm_layer
#         norm_fuse = norm_fuse or norm_layer
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        self.extra_block2 = nn.ModuleList([Block(
            dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,   ###cur+1改成cur + i
            sr_ratio=sr_ratios[1])
            for i in range(depths[1])])
        ...
        print("=== FFM Module Class Check ===")
        for i, ffm in enumerate(self.FFMs):
            print(f"FFM[{i}] =", ffm.__class__.__name__)
...
#             drop_rate=0.0,
#             drop_path_rate=0.1,
#             **kwargs          # ★ 關鍵：把 builder 傳來的 norm_fuse 帶上去
#         )
class mit_b2(RGBXTransformer):
    def __init__(self, fuse_cfg=None, **kwargs):
