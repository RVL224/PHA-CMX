...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        elif cfg.decoder == 'deeplabv3_pha':
            logger.info('Using Decoder: DeepLabV3_PHA (with attention)')
            from .decoders.deeplabv3_pha import DeepLabV3Plus as Head
            self.decode_head = Head(in_channels=self.channels, num_classes=cfg.num_classes, norm_layer=norm_layer)
            from .decoders.fcnhead import FCNHead
            self.aux_index = 2
            self.aux_rate = 0.4
    ...
    # # ✅ Debug：檢查每層 feature shape
    #     for i, f in enumerate(features):
    #         print(f"[builder.py] features[{i}] shape: {f.shape}")

    # ✅ 傳入整個 features 給 decoder（不要只取最後一層）
